$(document).ready(function() {
	...
	$('.clonedInput:not(:first-child), #spout-1-dimensions > *,#js-warning').remove();
	// insert the 'add another spout' button
	$('#edit-field-spout').append(addSpoutButton);

	// add the dimension fields apprpriate to the type of spout
	$('input.spout-type').click(function() {
		if ($(this).val() == 'flag-bag') {
			$('#spout-1-dimensions > *').remove();
			$('#spout-1-dimensions').append(spoutWidthField);
		} else if ($(this).val() == '4-sided-bag') {
			$('#spout-1-dimensions > *').remove();
			$('#spout-1-dimensions').append(spoutWidthField + spoutHeightField);
		} else {
			$('#spout-1-dimensions > *').remove();
			$('#spout-1-dimensions').append(spoutDiameterField);
		}
		;
	});
	...
	$('#btnAdd').click(function() {
		...
		var newElem = $('#spout-' + num).clone().attr('id', newSpoutID);

		// manipulate the name/id values of the input
		// inside the new element
		newElem.children(':first').attr('id', newSpoutID).attr('name', newSpoutID);
